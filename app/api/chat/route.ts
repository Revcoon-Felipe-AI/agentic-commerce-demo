...
function writeCostData(
  writer: UIMessageStreamWriter,
  payload: { model: string; inputTokens: number; outputTokens: number; costUSD: number },
): void {
  // Cast required because the generic defaults to UIMessage which types data as `unknown`.
  // The client reads this back via onData and re-validates shape with isTurnCostData.
  ;(writer.write as (chunk: unknown) => void)({
    type: 'data-turn-cost',
    data: payload,
  })
}
      ...
      execute: ({ writer }) => {
        const result = streamText({
          ...
          onFinish: ({ usage }: { usage: LanguageModelUsage }) => {
            ...
            writeCostData(writer, { model: route.modelId, inputTokens, outputTokens, costUSD })
            logTurnTelemetry({
